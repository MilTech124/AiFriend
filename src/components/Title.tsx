...
function Title({ setMessages }: Props) {
  const [isResetting, setIsResettiing] = useState(false);

  //RESET CONVERSATION
  const resetConversation = async () => {
    setIsResettiing(true);
  ...
  return (
    <div className=" flex justify-between items-center p-4 w-full bg-gray-900 text-white font-bold shadow">
      ...
      <button
        ...
        className={
          "transition-all duration-300 text-blue-300 hover:text-pink-500" +(isResetting ? " animate-pulse" : "")
        }
      >
        <svg
